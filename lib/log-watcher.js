 ...
 * @param pattern RegExp against which each new log entry will be matched to extract the log fields. It can also be
 *          a function that receives the log entry and return an array with the field values (or null if the log
 ...
 *          - autoStart: If true, the log watcher will starts watching the log file
...
var LogWatcher = function LogWatcher(source, pattern, fieldNames, opts) {
  ...
  this.opts = opts || {};
  this.opts.polling = opts.polling || false;
  this.opts.interval = opts.interval || 100;
  this.opts.allowPatternViolations = opts.allowPatternViolations || false;
  this.opts.retainedLogTimeout = opts.retainedLogTimeout || 300;
...
LogWatcher.prototype._parseLogs = function _parseLogs(data) {
  ...
  lines.forEach(function(line) {
    ...
    // Extract field values, either using a reg exp or calling a function
    var fieldsValues = null;
    if (self.pattern instanceof Function) {
      fieldsValues = self.pattern(line);
    } else {
      fieldsValues = line.trim().match(self.pattern);
      if (fieldsValues) {
    ...
    if (fieldsValues === null) {
      ...
      if (self.opts.allowPatternViolations && self._logs.length) {
    ...
    for (var i = 0; i < fieldsValues.length; i++) {
      log[self.fieldNames[i]] = fieldsValues[i];
    }
